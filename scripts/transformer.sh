...
function start_neuron() {
  ...
  log ""
  log "=== start Neuron ==="
  COMMAND="python3 transformer/main.py --identity $IDENTITY --serve_address $SERVE_ADDRESS --bind_address $BIND_ADDRESS --port $PORT --eosurl $EOSURL --logdir $LOGDIR"
  log "$COMMAND"
  eval $COMMAND &
  NueronPID=$!
}
